...
todoExpress.get('/todo/:id', (req, res) => TodoData.findOne(req.params.id).then(todo => res.json(todo)).catch((err) => res.status(500).json(err)));
...
todoExpress.put('/todo/:id', (req, res) => TodoData.update(req.params.id, req.body).then(() => res.sendStatus(201)).catch((err) => res.status(500).json(err)));
